...
export default function AdminDashboard({ 
  ...
  const handleAddCoachSubmit = async (e: React.FormEvent) => {
    ...
    let globalPricingPackages: PricingPackage[] = (pricingPackages && pricingPackages.length > 0) ? pricingPackages : [];
    if (globalPricingPackages.length === 0 && settings.pricing_packages) {
      try {
    ...
    <div className="flex flex-col lg:flex-row gap-6 relative">
      ...
      `}>
        <div className="space-y-3.5">
          ...
          <div className="lg:hidden flex justify-between items-center pb-2.5 border-b border-slate-100">
            <div className="flex items-center gap-2">
              <div className="p-1.5 bg-cyan-50 text-cyan-600 rounded-xl">
          ...
          <div className="hidden lg:flex items-center gap-2 pb-2.5 border-b border-slate-100">
            <div className="p-1.5 bg-cyan-50 text-cyan-600 rounded-xl">
              <ShieldCheck className="w-4 h-4" />
            </div>
            <div>
          ...
          <nav className="space-y-0.5">
            ...
            >
              <div className="flex items-center gap-2 min-w-0 flex-1 pr-1">
                <div className={`p-1 rounded-lg shrink-0 transition-colors ${activeTab === 'verifikasi' ? 'bg-white/15 text-white' : 'text-amber-600 bg-amber-50'}`}>
                  <CreditCard className="w-3.5 h-3.5" />
                </div>
                <span className="truncate text-xs leading-tight">Verifikasi Pembayaran</span>
              </div>
              {pendingPayments.length > 0 && (
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-black tracking-wide font-mono shrink-0 ml-1 ${
            ...
            >
              <div className="flex items-center gap-2 min-w-0 flex-1 pr-1">
                <div className={`p-1 rounded-lg shrink-0 transition-colors ${activeTab === 'peserta' ? 'bg-white/15 text-white' : 'text-cyan-600 bg-cyan-50'}`}>
            ...
            >
              <div className="flex items-center gap-2 min-w-0 flex-1 pr-1">
                <div className={`p-1 rounded-lg shrink-0 transition-colors ${activeTab === 'pelatih' ? 'bg-white/15 text-white' : 'text-teal-600 bg-teal-50'}`}>
            ...
            >
              <div className="flex items-center gap-2 min-w-0 flex-1 pr-1">
                <div className={`p-1 rounded-lg shrink-0 transition-colors ${activeTab === 'absensi_coach' ? 'bg-white/15 text-white' : 'text-rose-600 bg-rose-50'}`}>
            ...
            >
              <div className="flex items-center gap-2 min-w-0 flex-1 pr-1">
                <div className={`p-1 rounded-lg shrink-0 transition-colors ${activeTab === 'reminder' ? 'bg-white/15 text-white' : 'text-indigo-600 bg-indigo-50'}`}>
            ...
            >
              <div className="flex items-center gap-2 min-w-0 flex-1 pr-1">
                <div className={`p-1 rounded-lg shrink-0 transition-colors ${activeTab === 'events' ? 'bg-white/15 text-white' : 'text-rose-600 bg-rose-50'}`}>
            ...
            >
              <div className="flex items-center gap-2 min-w-0 flex-1 pr-1">
                <div className={`p-1 rounded-lg shrink-0 transition-colors ${activeTab === 'kolam_renang' ? 'bg-white/15 text-white' : 'text-cyan-600 bg-cyan-50'}`}>
            ...
            {!isOperator && (
              <div className="space-y-0.5">
                ...
                >
                  <div className="flex items-center gap-2 min-w-0 flex-1 pr-1">
                    <div className={`p-1 rounded-lg shrink-0 transition-colors ${activeTab === 'laporan' || activeTab === 'laporan_coachs' ? 'bg-emerald-600 text-white' : 'text-emerald-600 bg-emerald-50'}`}>
                      <BarChart2 className="w-3.5 h-3.5" />
                    </div>
                    <span className="truncate text-xs leading-tight">Laporan</span>
                  </div>
                  <ChevronRight className={`w-3.5 h-3.5 text-slate-400 transition-transform ${isLaporanGroupOpen ? 'rotate-90 text-emerald-600' : ''}`} />
                </button>

                {isLaporanGroupOpen && (
                  <div className="pl-3.5 pr-1 space-y-0.5 py-0.5 border-l-2 border-emerald-200/60 ml-3">
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className={`w-full px-2.5 py-1 rounded-lg text-[11px] font-bold transition flex items-center gap-1.5 text-left cursor-pointer ${
                        activeTab === 'laporan'
            ...
            {!isOperator && (
              <div className="space-y-0.5">
                ...
                >
                  <div className="flex items-center gap-2 min-w-0 flex-1 pr-1">
                    <div className={`p-1 rounded-lg shrink-0 transition-colors ${activeTab === 'audit_logs' || activeTab === 'pengaturan' || activeTab === 'paket_harga' ? 'bg-violet-600 text-white' : 'text-violet-600 bg-violet-50'}`}>
                      <Settings className="w-3.5 h-3.5" />
                    </div>
                    <span className="truncate text-xs leading-tight">Konfigurasi</span>
                  </div>
                  <ChevronRight className={`w-3.5 h-3.5 text-slate-400 transition-transform ${isKonfigurasiGroupOpen ? 'rotate-90 text-violet-600' : ''}`} />
                </button>

                {isKonfigurasiGroupOpen && (
                  <div className="pl-3.5 pr-1 space-y-0.5 py-0.5 border-l-2 border-violet-200/60 ml-3">
                    <button
                      type="button"
                      onClick={() => {
        ...
        {activeTab === 'penjadwalan' && (
          <div className="space-y-6">
            ...
            {showScheduleModal && (() => {
              ...
              const selectedCoach = coaches.find(c => c.id === scheduleFormCoachId);
                ...
                <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs">
                  <div className="relative bg-white rounded-3xl border border-slate-100 shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col my-auto">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500">
                          {scheduleModalMode === 'add'
                            ? `Atur hari, jam, dan lokasi kolam renang untuk ${selectedCoach ? `Coach ${selectedCoach.name}` : 'Pelatih'}`
                            : `Edit data jadwal ${selectedCoach ? `Coach ${selectedCoach.name}` : ''} (ID #${editingScheduleId})`}
                        </p>
                      </div>
                      <button
                        type="button"
                    ...
                    <form onSubmit={handleSaveSchedule} className="p-6 space-y-4 text-xs text-slate-700">
                      {/* Top: Locked Package & Coach Info Banner */}
                      <div className="bg-gradient-to-r from-cyan-50 via-blue-50 to-indigo-50 border border-cyan-200/80 p-4 rounded-2xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 shadow-2xs">
                        <div className="flex items-center gap-3.5">
                          {selectedCoach?.photo ? (
                            <img
                              src={getMediaUrl(selectedCoach.photo)}
                              alt={selectedCoach.name}
                              className="w-12 h-12 rounded-full object-cover border-2 border-white shadow-sm shrink-0"
                            />
                          ) : (
                            <div className="w-12 h-12 rounded-full bg-cyan-700 text-white font-black flex items-center justify-center text-sm shadow-sm shrink-0">
                          ...
                          <div>
                            ...
                            <h4 className="text-sm font-black text-cyan-950 mt-0.5 flex items-center gap-1.5">
                              <span>Coach {selectedCoach?.name || 'Pelatih Terpilih'}</span>
                        ...
                        <div className="text-left sm:text-right bg-white/90 border border-cyan-100 px-3.5 py-2 rounded-xl shrink-0">
                          ...
                          <span className="text-[9px] text-slate-400 block font-medium">(Terkunci dari Paket)</span>
                        </div>
                      </div>
                      ...
                      {/* 2-Column Wide Grid */}
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Left Column: Hari & Kolam */}
                        <div className="space-y-3">
                          {/* 1. Pilih Hari */}
                          <div className="space-y-1">
                            <label className="font-extrabold text-slate-800 block">
                              1. Pilih Hari Latihan <span className="text-rose-500">*</span>
                            </label>
                            <select
                          ...
                          {/* 2. Pilih Kolam Renang */}
                          <div className="space-y-1">
                            <label className="font-extrabold text-slate-800 block">
                              2. Pilih Lokasi Kolam Renang <span className="text-rose-500">*</span>
                            </label>
                            <select
                        ...
                        <div className="space-y-2.5 bg-slate-50 p-4 rounded-2xl border border-slate-200/80 flex flex-col justify-between">
                          <div>
                            <label className="font-extrabold text-slate-800 block mb-1.5">
                              3. Pilih Rentang Jam Latihan (WIB) <span className="text-rose-500">*</span>
                            </label>
